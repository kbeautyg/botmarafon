...
def test_смотрящий_день_и_молчун_считаются_отдельно():
    u"""С 20.09.2026 кнопки уходят вместе с записью, и открытый вопрос сам
    по себе больше не значит «молчит». Пока в очереди ждёт напоминание,
    человек просто смотрит день; молчуном он становится после него. Раньше
    строка «получили N-й день» была недостижима, а «не ответили» раздувалась
    всеми, кто в эту минуту включил видео (аудит 20.09.2026)."""
    _человек(40); db.set_poll(40, 'day2'); db.log_event(40, 'day', 2)
    db.add_job(40, 'after_day2', 0, NOW + 7200)          # смотрит, напоминание впереди
    _человек(41); db.set_poll(41, 'day2'); db.log_event(41, 'day', 2)  # напоминание ушло
