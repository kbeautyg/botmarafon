...
async def test_заявки_показывают_и_нажавших_купить_и_людей_с_сайта():
    u"""С 23.09.2026 «/заявки» — про нажавших «купить» (Павел: «где блин
    эти заявки, что нажали купить?»). Люди с сайта, кому марафон не
    уходил, — вторым сообщением и только админу: по кнопке им уйдёт марафон."""
    _прежний(1, name=u'Merunas')
    сообщение = FakeMessage(text='/zayavki', user=FakeUser(ADMIN))
    await admin.on_buys(сообщение)
    assert u'нажал' in сообщение.answers[0].lower()
    assert u'Merunas' in сообщение.answers[1]
