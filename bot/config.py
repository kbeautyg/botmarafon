...
# Оплата по кнопкам покупки (Павел 17.09.2026): ссылка Точка Банка на
# «курс Энергоспортзал». Пустая — кнопка ведёт как раньше: заявка и
# «менеджер свяжется». Обучению ссылку пока не ставили.
GYM_PAY_URL = 'https://tochkaplace.com/ia/df12d7ce-070a-4243-8ba4-34ae84e86e94'
PAY_URLS = {
    'gym': os.getenv('GYM_PAY_URL', GYM_PAY_URL).strip(),
...
# Не ответил на «посмотрел?» — следующий день не приходит, пока не нажмёт «Да»
# или «Нет» (Павел 17.09.2026: «пока да или нет не нажали, следующий день им
# не приходил»). До этого через 12 часов бот сам вёл по ветке «нет».
# Решение заказчика, а не настройка: переменная POLL_FALLBACK_HOURS в
# Railway больше не читается — иначе старое значение 12 из .env.example
# молча вернуло бы прежнее поведение. Механика добивания в коде осталась:
# вернуть — поставить здесь число часов.
POLL_FALLBACK_HOURS = 0
