...
async def on_answer(call: CallbackQuery):
    ...
    # Вместе с ветками снимаем и цепочку вопроса. До 20.09.2026 это было
    # не нужно: кнопки приходили её последним шагом, и к моменту ответа она
    # была отработана. Теперь кнопки идут под записью дня — на два с
    # половиной часа раньше, — а цепочка продолжает тикать, и человеку,
    # который уже получил оффер, голосом Павла прилетает кружок «успел
    # посмотреть второй день?» (аудит 20.09.2026).
    db.drop_chains(user_id, tuple(branches.values()) + ('after_' + poll,))
