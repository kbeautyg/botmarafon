...
LABELS = {
    ...
    # AleX 18.09.2026 переделывает кнопки в рабочих чатах (около 20): у
    # каждого чата своя метка, чтобы видеть, из какого чата пришли.
    'chat': u'Рабочий чат',
    # Реклама в Telegram Ads (21.09.2026): специалисты ставят в объявление
...
def chat_links_report(username: str, count: int = 20) -> str:
    u"""Ссылки для кнопок в рабочих чатах — по своей на каждый чат.

    AleX 18.09.2026: «в каждом чате у кнопки должна быть своя ссылка,
    чтобы потом отслеживать, из какого чата пришли в марафон». Метка
    сохраняется за человеком при первом входе и дальше видна везде:
    в /кто, в сводках, в разделе «Источники» и в выгрузке.
    """
    base = u'https://t.me/%s?start=chat_' % username
